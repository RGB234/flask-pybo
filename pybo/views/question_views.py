...
def create():
    form = QuestionForm()
    # "post" 방식(저장하기 버튼) 요청이면 데이터 저장 후 질문 목록 페이지로 이동 즉, question_form.html 화면에서 "저장하기" 클릭시 동작
    ...
    # GET 방식 요청(질문 등록하기)이면 질문 등록 페이지 렌더링 즉, question_list.html 화면에서 "질문 등록하기"클릭시 동작
...
def modify(question_id):
    ...
    if request.method == 'POST': #질문 수정 후 저장하기 버튼을 눌렀을 때
        form = QuestionForm()
        if form.validate_on_submit():
            ...
            db.session.commit()
            return redirect(url_for('question.detail', question_id=question_id))
    else: #method = "GET"일때 - 질문 수정 버튼을 눌렀을 때
